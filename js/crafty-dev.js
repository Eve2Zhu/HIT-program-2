...
(function (Crafty, window, document) {
      ...
      search: function (rect, filter) {
        ...
        if (filter) {
          ...
          for (i = 0, l = results.length; i < l; i++) {
            ...
            if (
              !found[id] &&
              obj.x < rect._x + rect._w &&
              obj._x + obj._w > rect._x &&
              obj.y < rect._y + rect._h &&
              obj._h + obj._y > rect._y
            )
              found[id] = results[i];
          }
  ...
  Crafty.c("2D", {
          ...
          if (
            ...
            if (this.rotation !== this._rotation) {
              ...
              var mbr = this._mbr,
                moved = false;
              if (mbr) {
                //check each value to see which has changed
                if (this.x !== this._x) {
    ...
    _rotate: function (v) {
      var theta = -1 * (v % 360), //angle always between 0 and 359
        ...
        st = Math.sin(rad),
        o = { x: this._origin.x + this._x, y: this._origin.y + this._y };
      ...
      var x0 = o.x + (this._x - o.x) * ct + (this._y - o.y) * st,
        ...
        x2 =
          o.x + (this._x + this._w - o.x) * ct + (this._y + this._h - o.y) * st,
        y2 =
          o.y - (this._x + this._w - o.x) * st + (this._y + this._h - o.y) * ct,
    ...
    intersect: function (x, y, w, h) {
      ...
      return (
        obj._x < rect.x + rect.w &&
        obj._x + obj._w > rect.x &&
        obj._y < rect.y + rect.h &&
        obj._h + obj._y > rect.y
      );
    },
    ...
    containsPoint: function (x, y) {
      ...
      for (i = 0, j = p.length - 1; i < p.length; j = i++) {
        if (
          p[i][1] > y != p[j][1] > y &&
          x <
            ((p[j][0] - p[i][0]) * (y - p[i][1])) / (p[j][1] - p[i][1]) +
              p[i][0]
        ) {
          c = !c;
        }
  ...
  Crafty.c("DOM", {
    ...
    init: function () {
      ...
      this.bind("NewComponent", updateClass).bind(
        "RemoveComponent",
        updateClass
      );
    ...
    DOM: {
      ...
      inner: function (obj) {
        var rect = obj.getBoundingClientRect(),
          x =
            rect.left +
            (window.pageXOffset ? window.pageXOffset : document.body.scrollTop),
          y =
            rect.top +
            (window.pageYOffset
              ? window.pageYOffset
  ...
  (function testSupport() {
    ...
    support.css3dtransform =
      typeof document.createElement("div").style["Perspective"] !==
        "undefined" ||
      typeof document.createElement("div").style[
        support.prefix + "Perspective"
      ] !== "undefined";
  })();
  Crafty.extend({
    ...
    addEvent: function (ctx, obj, type, fn) {
      ...
      if (!this._events[id + obj + type + fn])
        this._events[id + obj + type + fn] = afn;
      else return;
    ...
    viewport: {
      ...
      follow: (function () {
        ...
        function change() {
          Crafty.viewport.scroll(
            "_x",
            -(this.x + this.w / 2 - Crafty.viewport.width / 2 - offx)
          );
          Crafty.viewport.scroll(
            "_y",
            -(this.y + this.h / 2 - Crafty.viewport.height / 2 - offy)
          );
  ...
  Crafty.c("Canvas", {
    ...
    draw: function (ctx, x, y, w, h) {
      ...
      var pos = {
          //inlined pos() function, for speed
          _x: this._x + (x || 0),
          _y: this._y + (y || 0),
          _w: w || this._w,
          _h: h || this._h,
  ...
  Crafty.c("Multiway", {
    ...
    multiway: function (speed, keys) {
      ...
      this.bind("KeyDown", function (e) {
        if (this._keys[e.key]) {
          this._movement.x =
            Math.round((this._movement.x + this._keys[e.key].x) * 1000) / 1000;
          this._movement.y =
            Math.round((this._movement.y + this._keys[e.key].y) * 1000) / 1000;
        ...
        .bind("KeyUp", function (e) {
          if (this._keys[e.key]) {
            this._movement.x =
              Math.round((this._movement.x - this._keys[e.key].x) * 1000) /
              1000;
            this._movement.y =
              Math.round((this._movement.y - this._keys[e.key].y) * 1000) /
              1000;
            this.trigger("NewDirection", this._movement);
          }
        })
    ...
    speed: function (speed) {
      for (var k in this._keyDirection) {
        ...
        this._keys[keyCode] = {
          x:
            Math.round(
              Math.cos(this._keyDirection[k] * (Math.PI / 180)) * 1000 * speed.x
            ) / 1000,
          y:
            Math.round(
              Math.sin(this._keyDirection[k] * (Math.PI / 180)) * 1000 * speed.y
            ) / 1000,
        };
      }
      return this;
  ...
  Crafty.c("SpriteAnimation", {
    ...
    drawFrame: function (e) {
      ...
      if (
        data.frame === data.reel.length &&
        this._frame.current === data.frameTime
      ) {
        data.frame = 0;
    ...
    image: function (url, repeat) {
      ...
      if (!this.img) {
        ...
        this.img.onload = function () {
          if (self.has("Canvas"))
            self._pattern = Crafty.canvas.context.createPattern(
              self.img,
              self._repeat
            );
          self.ready = true;
        ...
        this.ready = true;
        if (this.has("Canvas"))
          this._pattern = Crafty.canvas.context.createPattern(
            this.img,
            this._repeat
          );
  ...
  Crafty.DrawManager = (function () {
    ...
    return {
      ...
      onScreen: function (rect) {
        return (
          Crafty.viewport._x + rect._x + rect._w > 0 &&
          Crafty.viewport._y + rect._y + rect._h > 0 &&
          Crafty.viewport._x + rect._x < Crafty.viewport.width &&
          Crafty.viewport._y + rect._y < Crafty.viewport.height
        );
      },
      ...
      merge: function (set) {
        do {
          ...
          while (i < l) {
            ...
            if (
              i < l - 1 &&
              current._x < next._x + next._w &&
              current._x + current._w > next._x &&
              current._y < next._y + next._h &&
              current._h + current._y > next._y
            ) {
              merger = {
                _x: ~~Math.min(current._x, next._x),
                _y: ~~Math.min(current._y, next._y),
                _w:
                  Math.max(current._x, next._x) + Math.max(current._w, next._w),
                _h:
                  Math.max(current._y, next._y) + Math.max(current._h, next._h),
              };
              merger._w = merger._w - merger._x;
              merger._h = merger._h - merger._y;
              merger._w =
                merger._w == ~~merger._w ? merger._w : (merger._w + 1) | 0;
              merger._h =
                merger._h == ~~merger._h ? merger._h : (merger._h + 1) | 0;
      ...
      boundingRect: function (set) {
        ...
        while (i < l) {
          ...
          tmp = [
            current._x,
            current._y,
            current._x + current._w,
            current._y + current._h,
          ];
      ...
      draw: function draw() {
        ...
        for (i = 0, l = objs.length; i < l; ++i) {
          ...
          var area = ent._mbr || ent,
            ...
            w = ~~Math.min(
              area._w - x,
              rect._w - (area._x - rect._x),
              rect._w,
              area._w
            ),
            h = ~~Math.min(
              area._h - y,
              rect._h - (area._y - rect._y),
              rect._h,
              area._h
            );
  ...
  Crafty.c("particles", {
    ...
    _Particles: {
      ...
      initParticle: function (particle) {
        ...
        var newAngle =
          (this.angle + this.angleRandom * this.RANDM1TO1()) * (Math.PI / 180); // convert to radians
        var vector = this.vectorHelpers.create(
          Math.sin(newAngle),
          -Math.cos(newAngle)
        ...
        particle.sharpness =
          particle.sharpness > 100
            ? 100
            : particle.sharpness < 0
            ? 0
            : particle.sharpness;
      ...
      update: function () {
        ...
        while (this.particleIndex < this.particleCount) {
          ...
          if (currentParticle.timeToLive > 0) {
            ...
            currentParticle.direction = this.vectorHelpers.add(
              currentParticle.direction,
              this.gravity
            );
            currentParticle.position = this.vectorHelpers.add(
            ...
            // Update colours
            var r = (currentParticle.colour[0] +=
              currentParticle.deltaColour[0]);
            var g = (currentParticle.colour[1] +=
              currentParticle.deltaColour[1]);
            var b = (currentParticle.colour[2] +=
              currentParticle.deltaColour[2]);
            var a = (currentParticle.colour[3] +=
              currentParticle.deltaColour[3]);
      ...
      play: function (id, repeat) {
        ...
        for (; i < l; i++) {
          ...
          if (sound.ended || !sound.currentTime) {
            ...
            //if all sounds playing, try stop the last one
            sound.currentTime = 0;
            sound.play();
          }
        }
  ...
  Crafty.storage = (function () {
    ...
    function serialize(e) {
      if (JSON) {
        ...
        alert(
          "Crafty does not support saving on your browser. Please upgrade to a newer browser."
        );
        return false;
      }
    }
      ...
      return {
        open: function (gameName_n) {
          ...
          if (arguments.length == 1) {
            db = {
              save: openDatabase(
                gameName_n + "_save",
                "1.0",
                "Saves games for " + gameName_n,
                5 * 1024 * 1024
              ),
              cache: openDatabase(
                gameName_n + "_cache",
                "1.0",
                "Cache for " + gameName_n,
                5 * 1024 * 1024
              ),
            };
          } else {
          ...
          db["save"].transaction(function (tx) {
            tx.executeSql(
              "SELECT key, timestamp FROM data",
              [],
              function (tx, res) {
                var i = 0,
                  a = res.rows,
        ...
        save: function (key, type, data) {
          ...
          db[type].transaction(function (tx) {
            tx.executeSql(
              "CREATE TABLE IF NOT EXISTS data (key unique, text, timestamp)"
            );
            tx.executeSql(
              ...
              function (tx, results) {
                if (results.rows.length) {
                  tx.executeSql(
                    "UPDATE data SET text = ?, timestamp = ? WHERE key = ?",
                    [str, t, key]
                  );
                } else {
        ...
        load: function (key, type, callback) {
          ...
          db[type].transaction(function (tx) {
            tx.executeSql(
              "SELECT text FROM data WHERE key = ?",
              [key],
              function (tx, results) {
                if (results.rows.length) {
      ...
      return {
        ...
        save: function (key, type, data) {
          ...
          document.cookie =
            ...
            "_ts=" +
            t +
            "; expires=Thur, 31 Dec 2099 23:59:59 UTC; path=/";
        },
        ...
        load: function (key, type, callback) {
          ...
          var reg = new RegExp(gameName + "_" + key + "=[^;]*"),
            ...
            data = unserialize(
              result[0].replace(gameName + "_" + key + "=", "")
            );
  ...
  Crafty.extend({
    ...
    load: function (data, oncomplete, onprogress, onerror) {
      ...
      for (; i < l; ++i) {
        ...
        if (
          Crafty.support.audio &&
          (ext === "mp3" || ext === "wav" || ext === "ogg" || ext === "mp4")
        ) {
    ...
    modules: function (modulesRepository, moduleMap, oncomplete) {
        ...
        function $script(paths, idOrDone, optDone) {
          ...
          setTimeout(function () {
            each(paths, function (path) {
              ...
              id && (ids[id] = 1);
              create(
                !validBase.test(path) && scriptpath
                  ? scriptpath + path + ".js"
                  : path,
                callback
              );
      ...
      for (var i in moduleMap) {
        if (i.indexOf("http://") != -1) modules.push(i);
        else
          modules.push(
            modulesRepository +
              "/" +
  ...
  Crafty.math.Vector2D = (function () {
    ...
    Vector2D.prototype.equals = function (vecRH) {
      return (
        vecRH instanceof Vector2D && this.x == vecRH.x && this.y == vecRH.y
      );
    }; // equals( )
    ...
    Vector2D.prototype.getNormal = function (vecRH) {
      if (vecRH === undefined) return new Vector2D(-this.y, this.x); // assume vecRH is <0, 0>
