...
Game.getGameName = function () {
  ...
  if (location.pathname.indexOf("festival-tips")) {
    name = Game.getStringParamFromUrl("name", "painting");
  }
  return name;
};
...
Game.importGameScript = function () {
  ...
  } else if (Game.NAME == "ai") {
    script = '<script type="text/javascript" src="js/ai.js"></script>';
  } else {
    script = [
      ...
      '<script src="./js/planeWar.app.js"></script>',
    ].join("");
  }
...
document.write(
  '<script src="msg/dialogContent_' + Game.LANG + '.js"></script>\n'
);
// Load game JavaScript.
